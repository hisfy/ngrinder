...
public class FileEntryService {
	 ...
	 * @return entrys
	 */
	private Map<String, String> ignoreEntry(Map<String, String> entrys, String[] ignores) {
		for (String ignore : ignores) {
			entrys.remove(ignore);
		}
		return entrys;
	}
	 ...
	 */
	public Map<String, Object> convertToTemplate(String harContent, boolean removeStaticResource) throws JSONException, IOException {
		Map<String, Object> resultMap = newHashMap();
		Map<String, Object> paramMap = newHashMap();
		paramMap = getHARContentToTemplate(harContent, removeStaticResource);
	 ...
	 * @throws JSONException
	 */
	private HAR isRecorderContentType(HAR har) {
		boolean recorderType = false;
		HAR resultHar = new HAR();
		...
		resultHar.setLog(har.getLog());
		resultHar.getLog().setEntries(harEntrys);
		return resultHar;
	}
	 ...
	 *
	 * @param file HAR file
	 * @param removeStaticResource remove static resource
	 * @return
	 * @throws IOException
	 */
	public String loadHAR(MultipartFile file, boolean removeStaticResource) throws IOException {
		...
		TypeReference<HAR> typeReference = new TypeReference<HAR>() {};
		try {
			io = file.getInputStream();
			har = mapper.readValue(io, typeReference);
			if (removeStaticResource) {
				har = isRecorderContentType(har);
			}
		} catch (IOException e) {
			LOG.error("ERROR loadHAR IOException", e);
		} finally {
	 ...
	 * @throws JSONException
	 */
	private Map<String, Object> getHARContentToTemplate(String harContent, boolean removeStaticResource) throws JSONException, IOException {
		HAR har = new HAR();
		String[] ignores = {"Host"};
		Map<String, Object> paramMap = newHashMap();
		...
		TypeReference<HAR> typeReference = new TypeReference<HAR>() {};
		try {
			har = mapper.readValue(harContent, typeReference);
		} catch (IOException e) {
			LOG.error("ERROR getHARContentToTemplate IOException", e);
			throw e;
		}
		...
		Map<String, String> commonHeader = ignoreEntry(extractCommonHeader(har), ignores);
		...
		for (HAR.HAREntry harEntry : har.getLog().getEntries()) {
			...
			request.setHeaders(ignoreEntry(tempHeader, ignores));
