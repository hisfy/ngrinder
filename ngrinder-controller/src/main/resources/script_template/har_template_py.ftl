...
<#if requests??>
	...
	perRequestHeaders = [];
		<#if request.postData?? && request.postData?size != 0>
			...
			<#list keys as key>
	postData.append(NVPair("${key}", "${request.postData[key]?replace("$", "\\$")}"))
			</#list>
		</#if>
		<#if request.headers?? && request.headers?size != 0>
			<#assign keys = request.headers?keys>
			<#list keys as key>
	perRequestHeaders.append(NVPair("${key}", "${request.headers[key]?replace("$", "\\$")}"))
			</#list>
		</#if>
	result = request.${request.method?default("GET")}("${request.url}",postData ,perRequestHeaders)
