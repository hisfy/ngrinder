...
<#if requests??>
	<#list requests as request>
		...
		perRequestHeaders = nvs([
			<#list keys as key>
			"${key}" : "${request.headers[key]?replace("$", "\\$")}" <#if key?has_next>,</#if>
			</#list>
		])
		</#if>
		HTTPResponse result = request.${request.method?default("GET")}("${request.url}",postData ,perRequestHeaders)
		assertThat(result.statusCode, is(${request.state}));
	</#list>
</#if>
